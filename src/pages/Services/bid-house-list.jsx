...
async function getHouse(token) {
    try {
        const { data } = await axios.get(`${baseUrl}/citizen_portal/bidding/house/`, {
            headers: {
...
export default function BidHouseList() {
    ...
    function loadData() {
        getHouse(userData.token).then((res) => {
            if (res.error) {
