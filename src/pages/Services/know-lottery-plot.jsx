...
async function getPlotDetail(token, id) {
    try {
        const { data } = await axios.get(`${baseUrl}/citizen_portal/lottery/plot/get?Id=${id}`, {
            headers: {
...
async function sendPlotBidRequest (token, username, id) {
    try {
        const { data } = await axios.post(`${baseUrl}/citizen_portal/lottery/plot/create`, {
            username, Id: id
        } ,{
            headers: {
...
export default function KnowLotteryPlot() {
    ...
    async function onSendRequestClick(e) {
        e.preventDefault();
        const { error } = await sendPlotBidRequest(userData.token, userData.username, params.id);
        if (error) {
            setAlert({ msg: "Error while sending request.", type: "error"});
        } else {
            setAlert({ msg: 'Lottery request has been saved.', type: "success"});
